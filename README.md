...
At present, gameR only supports discrete colour palettes. These palettes
are provided via the `gameR_cols()` function with the name of the
palette passed as an argument. For a full list of the palettes provided
by `gameR`, call `gameR_cols()` without any arguments.
